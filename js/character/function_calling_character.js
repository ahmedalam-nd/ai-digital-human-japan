...
export class FunctionCallingCharacter extends Character {
  constructor({
    characterId,
    firstName,
    lastName,
  ...
  async fetch({ humanText, apiKey, scene }) {
    return await fetch(Character.END_POINT_URL, {
      ...
      .catch((error) => {
        ...
        throw new Error(
          `Error in FunctionCallingCharacter.invokeMsg: ${error}`
        );
      })
      .then(async (json) => {
      ...
      .catch((error) => {
        return this.errorHandling({
          ...
          sceneId: scene.sceneId,
          occurredFunction: "FunctionCallingCharacter.fetch",
          error: error,
        });
      });
  }
