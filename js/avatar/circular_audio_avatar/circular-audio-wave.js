export class CircularAudioWave {
  ...
  loadAudio(filePath, play, muted) {
    console.log(filePath);
    this.filePath = filePath;
    this._setupAudioNodes(muted);
  ...
  loadFromTTS(filePath, callback, muted) {
    this.playing = false;
    this.context.close();
    this.sourceNode.buffer = null;
    this.offlineSource.buffer = null;
    ...
    this.offlineSource = this.offlineContext.createBufferSource();
    this.analyser = this.context.createAnalyser();
    this.loadAudio(filePath, true, muted);
    this.callback = callback;
  }
  _setupAudioNodes(muted) {
  ...
  // Stop the source node at the current time, mute it, and resume it from the stopped time after the specified time
  muteSourceNode() {
    const gainNode = this.context.createGain();
    gainNode.connect(this.context.destination);
    gainNode.gain.value = 0;
